<?php
    ...
    $nav_new_user = false;
    settype($nav_new_user, "boolean");

    $bt_userEdit_new = false;
    settype($bt_userEdit_new, "boolean");

    $bt_userEdit_delete = false;
    settype($bt_userEdit_delete, "boolean");

    $bt_userEdit_cancel = false;
    settype($bt_userEdit_cancel, "boolean");

    $bt_userEdit_save = false;
    settype($bt_userEdit_save, "boolean");

    $bt_userEdit_save_1 = false;
    settype($bt_userEdit_save_1, "boolean");

    $btn_avatar = false;
    settype($btn_avatar, "boolean");

    $btn_venusia = false;
    settype($btn_venusia, "boolean");

    $btn_actarus = false;
    settype($btn_actarus, "boolean");

    $btn_goldorak = false;
    settype($btn_goldorak, "boolean");

    $btn_monCompte = false;
    settype($btn_monCompte, "boolean");

    $newError = false;
    settype($newError, "boolean");
...
//***********************************************************************************************
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $bt_userEdit_save_1 = isset($_POST['bt_userEdit_save_1']) ? true : false;
        ...
        $_SESSION['message'] = isset($_POST['txt_userEdit_message']) ? filterInput('txt_userEdit_message') : '';
    }
...
//***********************************************************************************************
    ...
    if(($bt_userEdit_save || $bt_userEdit_save_1) && !$_SESSION['errorFormUser']){
        ...
        if(verifCsrf('csrfUSer')){
            ...
            if($_SESSION['newUser']){
                ...
                $MyUser->updateUser($_SESSION['dataConnect']['id_user']);
        ...
        if ($MyUser->deleteUser($_SESSION['id_user'])){
            ...
            if($_SESSION['dataConnect']['avatar'] === 'Member'){
    ...
    if($_SESSION['errorFormUser'] === false){
        ...
        if(!$_SESSION['newUser']){
            ...
            if($btn_monCompte){
                ...
                $theId = $MyUser->getId();
                $users = $MyUser->getCurrentUser($theId);
                $_SESSION['avatarConnect'] = $users['avatar'];
                $message = $_SESSION['message'];
